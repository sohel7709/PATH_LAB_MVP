...
const SuperAdminRevenue = () => {
  ...
  const fetchPlanChart = async () => {
    try {
      ...
      console.error('Error fetching plan chart:', err);
    }
  };
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <p className="text-gray-500 mb-8">Subscription-based revenue from all labs</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
          ...
          <div className="bg-white rounded-xl shadow p-5 border-l-4 border-teal-500">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-500">Labs Subscribed</p>
                <p className="text-2xl font-bold text-gray-900">{stats.totalLabsSubscribed}</p>
              </div>
              <BuildingOffice2Icon className="h-8 w-8 text-teal-500" />
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-xl shadow p-6">
            <h2 className="text-lg font-semibold text-gray-900 mb-4">Subscription Sales by Plan</h2>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  data={planChartData}
                ...
                <Tooltip formatter={(value) => [`${value} sales`, planChartData.find(d => d.subscriptions === value)?.planName]} />
                <Legend />
              </PieChart>
            </ResponsiveContainer>
          </div>
                ...
                {transactions.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="px-6 py-10 text-center text-gray-500">
                      No revenue transactions yet. Activate subscriptions to generate revenue.
                    </td>
                  </tr>
                ) : (
                  transactions.map((tx) => (
